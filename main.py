...
"""
Permet de manipuler les mots de la langue française

Modules :
    read_data(filename)
    ensemble_mots(filename)
    mots_de_n_lettres(mots, n)
    mots_avec(mots, s)
    cherche1(mots, start, stop, n)
    cherche2(mots, lstart, lmid, lstop, nmin, nmax)
"""
import random
...
def read_data(filename):
    ...
    with open(filename, 'r', encoding='utf-8') as file:
        mots = [mot.strip() for mot in file]
    return mots
...
def cherche2(mots, lstart, lmid, lstop, nmin, nmax):
    ...
    for mot in mots:
        if nmin <= len(mot) <= nmax:
            start_ok = any(mot.startswith(s) for s in lstart)
            stop_ok = any(mot.endswith(s) for s in lstop)
            mid_ok = any(s in mot and not mot.startswith(s) and not mot.endswith(s) for s in lmid)
            if start_ok and mid_ok and stop_ok:
                result.add(mot)
    return result
...
def main():
    """
    Fait des appels de test grâce aux modules
    """
    mots = read_data(FILENAME)
